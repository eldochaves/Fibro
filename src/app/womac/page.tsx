...
import { WomacForm } from "./WomacForm";
...
interface QrRow {
  ...
  summary: { pain?: number; stiffness?: number; function?: number } | null;
}

export default async function WomacPage() {
  const { supabase, user, isAdmin, profile } = await getContext();
  if (isAdmin) redirect("/admin");
  if (!isProfileComplete(profile)) redirect("/perfil");
  if (!(profile?.questionnaires ?? []).includes("womac")) redirect("/inicio");

  const { data } = await supabase
    .from("questionnaire_responses")
    .select("id, created_at, score, summary, by_doctor")
    .eq("questionnaire_key", "womac")
    .order("created_at", { ascending: false });
  const history = (data ?? []) as QrRow[];

  const freq = normalizeFrequency(profile?.questionnaire_freq?.["womac"]);
  const available = isAvailableNow(
    freq,
    history[0]?.created_at ?? null,
    profile?.questionnaire_requests?.["womac"]
  );
    ...
    <>
      ...
      <main className="mx-auto max-w-3xl px-4 py-6">
        ...
        <p className="mb-6 mt-1 text-sm text-navy-500">
          Pense em como você se sentiu nos <strong>últimos dias</strong>.
        </p>
                  ...
                  <div>
                    ...
                    {h.summary && (
                      <div className="mt-1 text-xs text-navy-400">
                        Dor {h.summary.pain}/20 · Rigidez {h.summary.stiffness}/8
                        · Função {h.summary.function}/68
                      </div>
                    )}
                  </div>
                  <span className="chip-teal">{h.score}/96</span>
                </li>
              ))}
            </ul>
          </div>
